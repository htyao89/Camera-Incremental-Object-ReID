    ...
    for c_ in range(len(cam_ids)):
        ...
        for epoch in range(max_epochs):
            ...
            if epoch == max_epochs-1:
                ...
                # dcc_loss_ewc = DCCLoss1(2048,num_cluster,weight=args.w, init_icc_feat=F.normalize(past_center_feat, dim=1).cuda(), init_ccc_feat = F.normalize(past_center_feat,dim=1).cuda())
                
                # for ii in range(iters):
                #     inputs = train_loader.next()
                #     inputs, labels, indexes = _parse_data(inputs)
                #     f_out = model(inputs)
                #     ewc_loss,_,_ = dcc_loss_ewc(f_out, labels)
                #     ewc_loss.backward()
