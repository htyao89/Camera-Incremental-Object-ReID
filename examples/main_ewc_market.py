    ...
    for c_ in range(len(cam_ids)):
        ...
        for epoch in range(max_epochs):
            ...
            if epoch%step_epoch==0:
                ...
                id_num = len(set(pseudo_labels)) - (1 if -1 in pseudo_labels else 0)
                
                if epoch==0:
                    num_cluster = len(set(pseudo_labels)) - (1 if -1 in pseudo_labels else 0)
                    curr_loss = CurrLoss(2048,num_cluster,weight= args.w, momentum = args.momentum, init_feat=F.normalize(curr_center_feat, dim=1).cuda())
                    cam_num[c] = num_cluster
                            
                if c_>0 and args.icm==True:
            ...
            if c_==0:
                ...
                if args.icm==False:
                    ...
                    for ii in range(iters):
                        ...
                        # add the EWC regularization
                        loss_reg_ewc=0.0
                        for (name,param),(_,param_old) in zip(model.named_parameters(),freeze_model_old.named_parameters()):
                            loss_reg_ewc+=torch.sum(fisher[name]*(param_old-param).pow(2))/2
                        loss_reg_ewc *= args.regular_weight
                        loss += loss_reg_ewc
                    ...
                    past_loss = PastLoss(2048,num_cluster_,weight= args.w, init_icc_feat=F.normalize(new_center, dim=1).cuda(),init_ccc_feat=F.normalize(new_center, dim=1).cuda())
                    ...
                    for ii in range(iters):
                        ...
                        loss_curr = curr_loss(f_out,labels)
                        loss_past = past_loss(f_out_,labels_)
                        loss = loss_curr + loss_past
            ...
            if epoch == max_epochs-1:
                ...
                if args.end_cm==True:
                    if c_==0:
                        ...
                        for i in range(c_num):
                            ...
                            if min_i == i:
                                past_center_feat[i_] = args.wu*past_center_feat[i_]+(1-args.wu)*curr_center_feat[i]
                ...
                loss_ewc = EWCLoss(2048,num_cluster,weight=args.w, init_icc_feat=F.normalize(past_center_feat, dim=1).cuda(), init_ccc_feat = F.normalize(past_center_feat,dim=1).cuda())

                for ii in range(iters):
                    inputs = train_loader.next()
                    inputs, labels, indexes = _parse_data(inputs)
                    f_out = model(inputs)
                    ewc_loss = loss_ewc(f_out, labels)
                    ewc_loss.backward()
        ...
        print(cam_prec)
        end_time_c = time.monotonic()
        print('Cam {} running time:{}'.format(c, timedelta(seconds=end_time_c - start_time_c)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wu', type=float, default=0)   
    # optimizer
