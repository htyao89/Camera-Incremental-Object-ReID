...
from dualclustercontrast.evaluators import Evaluator, extract_features, evaluate_loss
...
def main_worker(args):
    ...
    for c_ in range(len(cam_ids)):
        ...
        params_ = [{"params": [value]} for _, value in model_.named_parameters() if value.requires_grad]
        optimizer_ = torch.optim.Adam(params_, lr=curr_lr, weight_decay=args.weight_decay)
        
        evaluator = Evaluator(model)
        evaluator_ = Evaluator(model_)
        if c_==0:
            milestones=[15,30]
            max_epochs=30
            step_epoch=30
        ...
        lr_scheduler_ = torch.optim.lr_scheduler.MultiStepLR(optimizer_, milestones=milestones, gamma=0.1)
        for epoch in range(max_epochs):
            if epoch%step_epoch==0:
                ...
                if c_>0:
                    ...
                    for i in range(curr_center_feat.shape[0]):
                        ...
                        if min_i==i:
                            mask.append(i)
                            pseudo_labels_c[idx]=len(mask)-1
                            new_center.append(past_center_feat[i_].view(-1))
                        else:
                            pseudo_labels_c[idx]=-1
                    
                    new_center = torch.stack(new_center,dim=0)
                    mask = torch.from_numpy(np.asfarray(mask))
            ...
            if c_==0:
                ...
                for ii in range(iters):
                    ...
                    f_out = model(inputs)
                    loss,_, _ = dcc_loss(f_out,labels)
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                ...
                for ii in range(iters):
                    ...
                    loss_curr,_, _ = dcc_loss(f_out,labels)
                    loss_past,_,_ = dcc_loss_(f_out_,labels_)
                    losses_.update(loss_past.item())
                    loss = loss_curr + args.w*loss_past
                    ...
                    losses.update(loss.item())
                    if (ii+1)%iters==0:
                        print('{}->{}:{},{}'.format(epoch, ii, losses.avg,losses_.avg))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                            default=osp.join('/data/yht/data', 'data'))
